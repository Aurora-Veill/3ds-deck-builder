...
var ivl = []
@export var innerEnemy = preload("res://melee_enemy.tscn")
@export var numSpawn = 3
...
func _on_timer_timeout():
	for i in numSpawn:
		var en = innerEnemy.instantiate()
		en.position.x += randf_range(-2, 2)
		en.position.z += randf_range(-2, 2)
		en.target = target
